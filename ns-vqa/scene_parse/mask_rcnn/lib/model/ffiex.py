...
def _build_extension(ffi, cffi_wrapper_name, target_dir, verbose=True):
    try:
        tmpdir = tempfile.mkdtemp()
        ext_suf = '.pyd' if os.sys.platform == 'win32' else '.so'
        libname = str(cffi_wrapper_name) + ext_suf
...
def create_extension(name, headers, sources, verbose=True, with_cuda=False,
    ...
    return ffi, cffi_wrapper_name, target_dir

def build(ffi, cffi_wrapper_name, target_dir, verbose=1):
